...
/// NotebookLM's superscript chip and posts `.selectDocumentByID` on click.
struct CitationPill: View {
    ...
    var body: some View {
            ...
            Text("\(number)")
                ...
                .background(
                    Circle()
                        ...
                        .overlay(
                            Circle().strokeBorder(
                                Japandi.Colors.accentFallback.opacity(isHovered ? 0.0 : 0.35),
                                lineWidth: 0.5
                            )
                        )
                )
        ...
        .disabled(source == nil)
        .onHover { isHovered = $0 }
        .help(source.map { "Open \($0.documentTitle ?? "source")" } ?? "Source not available")
